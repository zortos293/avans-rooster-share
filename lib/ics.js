...
function classTodoToVevent(todo, index, stamp) {
  const block = todo.calendar_block || {};
  const startIso = block.start || `${todo.date}T19:00:00`;
  const endIso = block.end || `${todo.date}T21:00:00`;
  const startDate = startIso.slice(0, 10);
  const startTime = startIso.slice(11, 16);
  const endDate = endIso.slice(0, 10);
  const endTime = endIso.slice(11, 16);
  const summary = todo.action || "Huiswerk uit les";
  const description = [
    todo.source ? `Bron: ${todo.source}` : null,
    todo.course ? `Vak: ${todo.course}` : null,
    todo.file ? `Bestand: ${todo.file}` : null,
    todo.topics && todo.topics.length ? `Topics: ${todo.topics.join(", ")}` : null,
    todo.due_hint ? `Hint: ${todo.due_hint}` : null,
    "Klas: 290ICT1SEVSb",
  ]
    .filter(Boolean)
    .join("\n");
  const uid = `todo-${todo.date || "na"}-${index}-${String(summary)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")}@avans-rooster-share`;
  return [
    "BEGIN:VEVENT",
    `UID:${uid}`,
    `DTSTAMP:${stamp}`,
    `DTSTART;TZID=${TZID}:${toLocalStamp(startDate, startTime)}`,
    `DTEND;TZID=${TZID}:${toLocalStamp(endDate, endTime)}`,
...
function buildCalendar(lessons, opts = {}) {
  ...
  const classTodos = opts.classTodos || [];
  classTodos.forEach((todo, index) => {
    lines.push(...classTodoToVevent(todo, index, stamp));
  });
